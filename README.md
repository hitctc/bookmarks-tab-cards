...
* 键盘：`↑/↓` 选择、`Enter` 打开、`Esc` 清空搜索、`Cmd + K`（macOS）/ `Ctrl + K`（Windows）聚焦搜索
* 搜索视图：支持“卡片 / 列表”切换；`Cmd/Ctrl + Enter` 强制新标签打开
* 支持书签置顶：置顶书签在目录中优先显示，多条置顶按置顶时间升序
* 首页效率区：入口文件夹下新增“置顶”“最近打开”双区块
* 最近打开记录：按 `bookmarkId/openedAt/openCount` 本地持久化
* 缓存：优先读 `chrome.storage.local` 缓存秒开首屏，后台刷新书签
* 自动刷新：监听书签创建/删除/编辑/移动事件，防抖后自动刷新
* 设置：入口文件夹、主题（系统/浅色/深色）、打开方式（当前标签/新标签）
* 设置新增：自动刷新开关、搜索结果视图切换、清空最近打开记录
* 重复书签处理：支持按“名称重复/地址重复”分组并执行打开、删除、保留一条删除其余
* 卡片：顶部宽幅图标区（优先显示 favicon，失败则显示书签标题）
* 网格：支持设置每行卡片数量（2~9，默认 5）
...
手工关键路径：
* 新标签首页可见“置顶 + 最近打开”区块（入口目录下）
* 点击任意书签后，“最近打开”记录会更新
* 搜索支持卡片/列表切换，`Enter` 与 `Cmd/Ctrl + Enter` 行为正确
* 设置中的自动刷新开关生效（在 Chrome 书签管理器改动后能自动同步）
* 重复书签检查可执行处理（删除单条、保留一条删除其余）
