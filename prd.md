...
* 权限最小化：优先只用 `bookmarks` + `storage`；打开链接尽量走浏览器默认行为（不强依赖 `tabs` 权限）。
...
* 最少权限：优先只申请 `bookmarks`、`storage`；不申请 `host_permissions`。
* 不主动外联：插件页面加载时不主动请求任何第三方网络资源（favicon 优先走 `chrome://favicon2/`；失败则本地兜底）。
...
* 设置（轻量）：入口文件夹选择、打开方式（当前标签/新标签）、主题（跟随系统/浅色/深色）。
* 明确不做：置顶/常用统计、标签体系、站点截图、跨设备独立同步、书签管理（新增/编辑/移动）。
...
* 原则 2：最少权限原则（仅申请 `bookmarks`、`storage`；不采集、不上传；优先不引入 `tabs` 权限）。
* 原则 3：以键盘效率为核心（搜索框自动聚焦、回车打开、上下选择）。
* 原则 4：卡片展示要有稳定识别信息（标题 + 域名 + favicon）。
...
* 顶部：入口文件夹下拉 + 搜索框（默认聚焦），可选放置 Logo/名称。
* 中部：面包屑（当前文件夹路径）+ 卡片网格（书签卡片/文件夹卡片）。
...
* 插件内部索引结构（建议）：
  ...
  * BookmarkIndexItem（扁平化，供渲染与搜索使用）：
    ...
    * domain（从 url 解析）
    * faviconUrl（优先 `chrome://favicon2/?size=64&url=...`，失败则走本地图标兜底）
...
> 打开链接策略（优先）：使用浏览器默认跳转（`<a href>` / `window.location` / `window.open`）。若后续一定要 `chrome.tabs.*`，需评估是否值得引入 `tabs` 权限。
...
* 隐私承诺：
  ...
  * 插件页面加载不主动发起第三方网络请求（用户点击书签访问站点属于用户行为）
...
* AC2：默认展示“入口文件夹”的第一层内容；书签以卡片展示，至少包含标题与域名信息（favicon 可兜底）。
...
* AC6：点击书签卡片可打开对应 URL；`Ctrl/Cmd + Click` 或中键遵循浏览器默认在新标签打开（不强依赖额外权限）。
* AC7：书签变更后刷新页面能看到最新书签（v0.1 可接受“刷新触发更新”）。
* AC8：有缓存时，打开新标签页能先渲染缓存内容，再异步刷新为最新书签数据。
* AC9：设置可保存并生效（至少包含：入口文件夹选择、主题、默认打开方式）。
...
* 隐私与安全：
  * 插件页面加载不主动发起第三方网络请求（除非用户点击书签访问目标站点）
  * 权限遵循最小化原则（仅 `bookmarks` + `storage`）
* 可维护性：
...
* 风险 2：favicon 获取策略不稳定或跨域限制
  ...
  * 对策：优先使用 `chrome://favicon2/`（必要时 fallback 到 `chrome://favicon/`）；提供兜底为纯文本域名徽标。
...
* 风险 6：New Tab Override 与其他同类插件冲突（用户只能启用一个）
  ...
  * 对策：在 README/商店描述明确说明；页面内提供“冲突排查提示”（可选）。
...
* 打开策略：是否需要提供“默认新标签打开”的设置？若需要，是否接受引入 `tabs` 权限（或只用 `window.open`）？
