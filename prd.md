...
* 当前迭代重点是“常用即点开”：首页置顶 + 最近打开、搜索键盘闭环、自动刷新、重复书签可处理。
* 默认展示一个“入口文件夹”的书签卡片（建议默认：书签栏），搜索覆盖全部书签。
* 权限最小化：使用 `bookmarks` + `storage`；不申请 `host_permissions`。

> 近期实现（2026-02-27）：
> - 首页新增“置顶”与“最近打开”双区块；
> - 搜索支持卡片/列表切换，`Enter` 打开，`Cmd/Ctrl + Enter` 强制新标签；
> - 监听书签创建/删除/编辑/移动事件并自动刷新（防抖）；
> - 重复书签支持“打开、删除、保留一条删除其余”。
...
* NG1：不做“完整书签管理器”替代（批量导入导出、复杂规则清洗、可视化树编辑等）。
