...
* 不主动外联（默认）：页面打开时不主动请求第三方资源；仅在“站点预览图”开启或用户手动刷新某张预览图时，才会访问对应站点用于生成本地预览图缓存。
...
* 浏览：默认展示“入口文件夹”（建议默认：书签栏）下的**第一层**内容：
  * 书签：以卡片展示：
    * 顶部 16:9 图标区：优先显示 favicon；无 favicon 则显示书签标题兜底。
    * 下方信息区：书签标题 + 缩小的 URL。
...
> 变更说明（2026-01-04）：
> - 为提升卡片可识别性，支持“本地生成并缓存”的站点预览图：在卡片 16:9 区域中尝试加载网页，加载成功后截图生成预览图并缓存到用户浏览器本地；已有预览图时直接展示，不重复加载页面；支持手动刷新单张预览图。
> - 该能力不依赖任何服务器，不上传用户数据；仅使用本地存储。
...
* 权限清单（v0.1）：
  * `bookmarks`：读取/搜索本地书签
  * `storage`：缓存索引与保存用户设置
  * `tabs`：生成站点预览图时，用于对当前标签页截图裁剪（仅在用户开启/触发预览图时使用）
* 隐私承诺：
  * 不采集、不上传任何用户数据（包含书签内容与使用行为）
  * 默认不主动发起第三方网络请求；当用户开启“站点预览图”或点击单卡片的刷新预览按钮时，会访问目标站点用于生成并缓存本地预览图
  * 预览图仅存储在用户浏览器本地，不上传
...
* AC2：默认展示“入口文件夹”的第一层内容；书签以卡片展示，包含 16:9 图标区与信息区（标题 + URL）。
...
* AC10：站点预览图：开启后，卡片 16:9 区域可生成并展示本地缓存预览图；已缓存时不重复加载页面；支持单卡片刷新预览图。
...
* 隐私与安全：
  * 默认不主动发起第三方网络请求；开启/刷新“站点预览图”时会访问目标站点用于生成本地预览图缓存
  * 权限遵循最小化原则（`bookmarks` + `storage` + `tabs`；不申请 `host_permissions`）
* 可维护性：
...
* 风险 7：站点预览图可能被站点的 `X-Frame-Options`/`frame-ancestors` 限制导致加载失败

  * 对策：生成失败时自动回退到 favicon/标题兜底；提供“刷新预览”按钮手动重试；对失败做本地标记避免反复消耗资源（可选）。
